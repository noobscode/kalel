...
def start(argv):
    ...
    for opt, arg in opts:
        if opt == '-l':
            ...
            engine = arg
            if engine not in ("baidu", "bing", "crtsh","bingapi","dogpile", "google", "googleCSE","virustotal", "googleplus", "google-profiles","linkedin", "twitter", "vhost", "yahoo","netcraft","all"):
                usage()
                print "Invalid search engine, try with: baidu, bing, bingapi,crtsh, dogpile, google, googleCSE, virustotal, netcraft, googleplus, google-profiles, linkedin, twitter, vhost, yahoo, all"
                sys.exit()
            else:
                pass
    if engine == "google":
